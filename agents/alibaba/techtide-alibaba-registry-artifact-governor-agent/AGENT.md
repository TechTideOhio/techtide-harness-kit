...
> Agent for `techtide-alibaba-registry-artifact-governor`. Govern Alibaba Cloud Container Registry (ACR) - Enterprise Edition vs Personal Edition selection, image vulnerability scanning, namespace IAM least privilege, image retention policies, cross-region replication, and supply chain security posture.
...
Use this canonical agent only for `techtide-alibaba-registry-artifact-governor` work.
