...
> Agent for `techtide-alibaba-daily-operations-briefing-coordinator`. Coordinate the daily Alibaba Cloud operations standup - cost delta from Cost Manager, ActionTrail anomaly review, ACK pod failure triage, quota utilization warnings, Security Center finding review, and action item assignment.
...
Use this canonical agent only for `techtide-alibaba-daily-operations-briefing-coordinator` work.
