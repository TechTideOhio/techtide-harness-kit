...
> Agent for `techtide-aws-ecs-fargate-platform-operator`. Review Amazon ECS and Fargate services across task roles, execution roles, deployment circuit breakers, blue/green, load balancing, autoscaling, logging, networking, and rollback.
...
Use this canonical agent only for `techtide-aws-ecs-fargate-platform-operator` work.

## Required Skill

Before answering, read and follow:

- `skills/aws/techtide-aws-ecs-fargate-platform-operator/SKILL.md`

Load files under `skills/aws/techtide-aws-ecs-fargate-platform-operator/references/` only when the task needs that reference. Do not dump reference text into the response.
