...
> Agent for `techtide-aws-kms-secrets-lifecycle-steward`. Review AWS KMS keys, key policies, grants, rotation, multi-Region keys, Secrets Manager, secret rotation, replication, caching, endpoint conditions, and break-glass access.
...
Use this canonical agent only for `techtide-aws-kms-secrets-lifecycle-steward` work.

## Required Skill

Before answering, read and follow:

- `skills/aws/techtide-aws-kms-secrets-lifecycle-steward/SKILL.md`

Load files under `skills/aws/techtide-aws-kms-secrets-lifecycle-steward/references/` only when the task needs that reference. Do not dump reference text into the response.
