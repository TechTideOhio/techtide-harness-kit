...
> Agent for `techtide-gcp-waf-reliability-review`. Evaluates GCP workload reliability against the Well-Architected Framework reliability pillar.
...
Use this canonical agent only for `techtide-gcp-waf-reliability-review` work.
...
Load files under `skills/gcp/techtide-gcp-waf-reliability-review/references/` only when the task needs that reference.
