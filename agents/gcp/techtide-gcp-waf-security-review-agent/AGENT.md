...
> Agent for `techtide-gcp-waf-security-review`. Evaluates GCP workload security posture against the Well-Architected Framework security pillar.
...
Use this canonical agent only for `techtide-gcp-waf-security-review` work.
...
Load files under `skills/gcp/techtide-gcp-waf-security-review/references/` only when the task needs that reference.
