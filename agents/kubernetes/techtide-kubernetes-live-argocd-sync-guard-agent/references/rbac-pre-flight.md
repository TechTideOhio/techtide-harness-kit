# RBAC pre-flight self-check - Kubernetes Live Argo CD Sync Guard
...
Apply `references/least-privilege-rbac.yaml` (shipped with this agent) before invoking it. The manifest creates `ServiceAccount/techtide-argocd-sync-guard` in namespace `techtide-system`.
...
```bash
SA="system:serviceaccount:techtide-system:techtide-argocd-sync-guard"
...
## Domain-specific must-not-be-yes (Kubernetes Live Argo CD Sync Guard)

```bash
# Delete on Argo CD resources - rollback via apply -f baseline
kubectl auth can-i delete applications.argoproj.io --all-namespaces --as=$SA
kubectl auth can-i delete applicationsets.argoproj.io --all-namespaces --as=$SA
kubectl auth can-i delete appprojects.argoproj.io --all-namespaces --as=$SA
# AppProject and ApplicationSet writes - GitOps-only territory
kubectl auth can-i create appprojects.argoproj.io -n argocd --as=$SA
kubectl auth can-i patch appprojects.argoproj.io -n argocd --as=$SA
kubectl auth can-i create applicationsets.argoproj.io -n argocd --as=$SA
kubectl auth can-i patch applicationsets.argoproj.io -n argocd --as=$SA
# Argo CD control plane
kubectl auth can-i patch deployments -n argocd --as=$SA
kubectl auth can-i patch configmaps -n argocd --as=$SA
kubectl auth can-i get secrets -n argocd --as=$SA
```

## Domain-specific must-be-yes (Kubernetes Live Argo CD Sync Guard)

```bash
kubectl auth can-i list applications.argoproj.io -n argocd --as=$SA
kubectl auth can-i list applicationsets.argoproj.io -n argocd --as=$SA
kubectl auth can-i list appprojects.argoproj.io -n argocd --as=$SA
kubectl auth can-i patch applications.argoproj.io -n argocd --as=$SA
```
