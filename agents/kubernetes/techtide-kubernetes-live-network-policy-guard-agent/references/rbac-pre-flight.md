# RBAC pre-flight self-check - Kubernetes Live Network Policy Guard
...
Apply `references/least-privilege-rbac.yaml` (shipped with this agent) before invoking it. The manifest creates `ServiceAccount/techtide-network-policy-guard` in namespace `techtide-system`.
...
```bash
SA="system:serviceaccount:techtide-system:techtide-network-policy-guard"
...
## Domain-specific must-not-be-yes (Kubernetes Live Network Policy Guard)

```bash
# Cluster-wide policy writes - opt-in only; default refusal
kubectl auth can-i create ciliumclusterwidenetworkpolicies.cilium.io --as=$SA
kubectl auth can-i patch ciliumclusterwidenetworkpolicies.cilium.io --as=$SA
kubectl auth can-i delete ciliumclusterwidenetworkpolicies.cilium.io --as=$SA
# Delete on namespaced policies - rollback is via apply -f baseline, not delete
kubectl auth can-i delete ciliumnetworkpolicies.cilium.io --all-namespaces --as=$SA
kubectl auth can-i delete networkpolicies.networking.k8s.io --all-namespaces --as=$SA
# Cilium agent ConfigMap (cluster behavior)
kubectl auth can-i patch configmaps/cilium-config -n kube-system --as=$SA
```

## Domain-specific must-be-yes (Kubernetes Live Network Policy Guard)

```bash
kubectl auth can-i create ciliumnetworkpolicies.cilium.io --all-namespaces --as=$SA
kubectl auth can-i patch ciliumnetworkpolicies.cilium.io --all-namespaces --as=$SA
kubectl auth can-i create ciliumegressgatewaypolicies.cilium.io --all-namespaces --as=$SA
kubectl auth can-i create networkpolicies.networking.k8s.io --all-namespaces --as=$SA
kubectl auth can-i list ciliumnetworkpolicies.cilium.io --all-namespaces --as=$SA
kubectl auth can-i list ciliumendpoints.cilium.io --all-namespaces --as=$SA
```
