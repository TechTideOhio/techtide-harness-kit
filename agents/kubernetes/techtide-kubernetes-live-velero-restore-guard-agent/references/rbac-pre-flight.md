...
Apply `references/least-privilege-rbac.yaml` (shipped with this agent) before invoking it. The manifest creates `ServiceAccount/techtide-velero-restore-guard` in namespace `techtide-system`.
...
```bash
SA="system:serviceaccount:techtide-system:techtide-velero-restore-guard"
