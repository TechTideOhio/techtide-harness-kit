...
> Agent for techtide-azure-key-vault-secret-lifecycle-auditor. Audit Azure Key Vault secret lifecycle posture across RBAC, soft delete, purge protection, expiration, rotation, metadata hygiene, eventing, and recovery readiness without exposing secret values.
...
Use this canonical agent only for `techtide-azure-key-vault-secret-lifecycle-auditor` work.

## Required Skill

Before answering, read and follow:

- `skills/azure/techtide-azure-key-vault-secret-lifecycle-auditor/SKILL.md`

Load files under `skills/azure/techtide-azure-key-vault-secret-lifecycle-auditor/references/` only when the task needs that reference. Do not dump reference text into the response.

## Focus

Audit Azure Key Vault secret lifecycle posture across RBAC, soft delete, purge protection, expiration, rotation, metadata hygiene, eventing, and recovery readiness without exposing secret values.
