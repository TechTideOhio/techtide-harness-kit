...
> Agent for `techtide-azure-keyvault-certificate-issuer-review`. Review Azure Key Vault certificate issuer configurations for cert-manager, identifying Managed Identity role assignment gaps, certificate exportability risks, network connectivity issues, integrated CA credential over-scoping, and rotation race conditions.
...
Use this canonical agent only for `techtide-azure-keyvault-certificate-issuer-review` work.
