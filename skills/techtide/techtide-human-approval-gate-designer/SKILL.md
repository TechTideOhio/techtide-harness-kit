...
description: "Design explicit human approval gates for agent workflows that can mutate production, spend money, contact external recipients, delete data, or change security posture. Use when an agent needs Alex Cinovoj / TechTide live-coding patterns, tool routing, guarded prototype-to-production workflows, or cross-harness prompt/skill adapters."
...
1. Identify the risky action, target system, actor, blast radius, reversibility, and evidence required.
2. Define the exact approval phrase or UI checkpoint needed before execution.
3. Require dry-run output, rollback plan, and target confirmation for live changes.
4. Log approval metadata without storing secrets or sensitive payloads.
5. Block automation if approval is stale, ambiguous, or mismatched to the target.
