...
1. Resolve absolute target paths before move, delete, copy, or recursive operations.
2. Use one shell end-to-end for filesystem mutations and prefer native PowerShell cmdlets on Windows.
3. Treat background services, network listeners, and process restarts as operational changes.
4. Preview destructive actions and require explicit approval when the target is broad or ambiguous.
5. Record commands run and verification results without exposing local secrets.
