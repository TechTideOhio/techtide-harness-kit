...
description: "Split substantial engineering work across agents or worktrees with disjoint ownership, clear contracts, validation checkpoints, and integration review. Use when an agent needs Alex Cinovoj / TechTide live-coding patterns, tool routing, guarded prototype-to-production workflows, or cross-harness prompt/skill adapters."
...
# TechTide Multi-Agent Worktree Dispatch
...
1. Decompose work by independent files, modules, or verification surfaces.
2. Assign each agent a bounded write scope, task contract, and expected output.
3. Keep the critical path local when immediate decisions depend on the result.
4. Integrate by reviewing diffs, running combined tests, and resolving overlapping assumptions.
5. Close or archive transient agent artifacts when they are no longer needed.
