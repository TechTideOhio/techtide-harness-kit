---
name: techtide-replit-fullstack-bootstrap
description: "Bootstrap Replit projects with a clear app contract, environment variable model, test loop, deployment boundary, and migration path back into a durable repository. Use when an agent needs Alex Cinovoj / TechTide live-coding patterns, tool routing, guarded prototype-to-production workflows, or cross-harness prompt/skill adapters."
...
# TechTide Replit Full-Stack Bootstrap

Bootstrap Replit projects with a clear app contract, environment variable model, test loop, deployment boundary, and migration path back into a durable repository.
...
1. Define runtime, package manager, app entrypoint, persistence layer, and public/private environment variables.
2. Generate only the smallest vertical slice that can run in Replit and prove the app loop.
3. Add a health check, smoke test, and README commands before expanding features.
4. Keep credentials in Replit secrets and record only variable names in code or docs.
5. Plan repo migration once the prototype needs CI, private deployment, custom infrastructure, or deep tests.
