...
- NetworkPolicy / CiliumNetworkPolicy / CiliumClusterwideNetworkPolicy content review → `techtide-cilium-network-policy-review`.
- Istio mesh policy / ambient L7 → `techtide-istio-ambient-mesh-review`.
- Live mutations on policy objects → `kubernetes-live-network-policy-guard`, `kubernetes-live-mesh-policy-guard`.
- Pod `securityContext`, capabilities, host networking → `techtide-kubernetes-pod-spec-review`.
